    ...
    public struct FirestormDocumentReference
    {
        ...
        private async Task DocumentOpsInternal(object documentDataToSet, SetOption setOption, (string fieldPath, object[] objs) arrayOperation = default)
        {
            ...
            if (documentDataToSet != null)
            {
                ...
                writeTransformTimestamp = new WriteTransform {transform = new DocumentTransform {document = documentPath, fieldTransforms = timestampFieldTransforms}};
            }
            ...
            if (arrayOperation != default)
            {
                ...
                {
                    arrayTransform = new FieldTransformArrayAppend {fieldPath = arrayOperation.fieldPath, appendMissingElements = new AppendArray {values = valueToArrayTransform}};
                }
                else if (setOption == SetOption.ArrayRemove)
                {
                    arrayTransform = new FieldTransformArrayRemove {fieldPath = arrayOperation.fieldPath, removeAllFromArray = new RemoveArray {values = valueToArrayTransform,}};
                }

                var writeTransformArray = new WriteTransform {transform = new DocumentTransform {document = documentPath, fieldTransforms = new ITransform[] {arrayTransform},}};
            ...
            else
            {
                ...
                var writeUpdate = new WriteUpdate {updateMask = new DocumentMask {fieldPaths = mergedFields.ToArray()}, update = doc,};
                commit = new CommitUpdate {writes = (timestampFieldTransforms != null && timestampFieldTransforms.Length == 0) ? new WriteUpdate[] {writeUpdate} : new IWrite[] {writeUpdate, writeTransformTimestamp},};
            }
