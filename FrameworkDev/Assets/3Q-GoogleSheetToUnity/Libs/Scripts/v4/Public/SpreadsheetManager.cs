...
namespace GoogleSheetsToUnity
{
    ...
    public partial class SpreadsheetManager
    {
        ...
        public static void ReadPublicSpreadsheet(GSTU_Search searchDetails, OnSpreedSheetLoaded callback)
        {
            ...
            {
                new Task(Read(new WWW(sb.ToString()), searchDetails.worksheetName, searchDetails.titleColumn, searchDetails.titleRow, callback));
            }
#if UNITY_EDITOR
            else
            {
                EditorCoroutineRunner.StartCoroutine(Read(new WWW(sb.ToString()), searchDetails.worksheetName, searchDetails.titleColumn, searchDetails.titleRow, callback));
            }
#endif
        }
        ...
        /// <returns></returns>
        static IEnumerator Read(WWW www, string name, string titleColumn, int titleRow, OnSpreedSheetLoaded callback)
        {
            yield return www;
            ...
            GstuSpreadSheet spreadSheet = new GstuSpreadSheet(responce, name, titleColumn, titleRow);
